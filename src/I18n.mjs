...
export class I18n {
  ...
  /**
   * @param {{ defaultLocale?: string, storageKey?: string, bundlesBasePath?: string }} [options={}]
   */
  constructor(options = {}) {
    ...
    this.#bundlesBasePath = String(options.bundlesBasePath || this.#bundlesBasePath)
  }
  ...
  async #loadTranslations() {
    const loadTasks = I18n.SUPPORTED_LOCALES.map(async (locale) => {
      const response = await fetch(`${this.#bundlesBasePath}/${locale}.json`, { cache: 'no-store' })
      if (!response.ok) {
