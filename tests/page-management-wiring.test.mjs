...
test('main runtime supports page reorder, insert, delete, and append mutations', async () => {
  ...
  assert.match(source, /async function insertPdfFilesAfterPage\(files, afterPage\)/)
  assert.match(source, /async function buildReorderedPdf\(sourcePdfDoc, orderedPageIndices\)/)
  assert.match(source, /function remapPlacementsForPageBlockInsertion\(placements, afterPage, insertedPageCount\)/)
  assert.match(source, /async function loadPdfInsertionSources\(files\)/)
  assert.match(source, /PDFDocument\.create\(\)/)
  assert.match(source, /reorderedIndices\.splice\(fromPage - 1, 1\)/)
  assert.match(source, /reorderedIndices\.splice\(toPage - 1, 0, movedIndex\)/)
